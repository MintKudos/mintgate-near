...
fn create_and_claim_collectibles() {
    let Sim { nft, alice, bob, charlie, admin, .. } = &init(0, "1/1000", "70/100", "35/100");
    let users = [alice, bob, charlie];

    create_collectible(nft, admin, alice, gate_id(1), 10, "1/0")
        .failure(mg_core::CorePanics::ZeroDenominatorFraction {}.msg());

    create_collectible(nft, admin, alice, gate_id(1), 10, "2/1")
        .failure(mg_core::CorePanics::FractionGreaterThanOne {}.msg());

    create_collectible(nft, admin, bob, gate_id(1), 10, "0/10").failure(
    ...
    create_collectible(nft, admin, bob, gate_id(1), 10, "8/10").failure(
    ...
    create_collectible(nft, admin, bob, gate_id(1), 10, "70/100").failure(
    ...
    create_collectible(nft, admin, charlie, gate_id(1), 0, "1/10")
        .failure(mg_nft::Panic::ZeroSupplyNotAllowed { gate_id: gate_id(1).to_string() }.msg());

    create_collectible(nft, alice, charlie, gate_id(1), 10, "1/10")
        .failure(Panic::AdminRestrictedOperation.msg());

    let n = 4;
    for k in 1..=n {
        create_collectible(nft, admin, alice, gate_id(k), 4, "10/100").unwrap();
        create_collectible(nft, admin, bob, gate_id(k + n), k * 10, "10/100").unwrap();
        create_collectible(nft, admin, charlie, gate_id(k), k * 10, "10/100")
            .failure(mg_nft::Panic::GateIdAlreadyExists { gate_id: gate_id(k).to_string() }.msg());
        create_collectible(nft, admin, charlie, gate_id(k + 2 * n), k * 10, "10/100").unwrap();
...
fn nft_approve_and_revoke_tokens() {
    let Sim { nft, markets, fake_market, alice, bob, charlie, admin, .. } =
...
fn batch_approve_a_few_tokens() {
    let Sim { nft, markets, alice, bob, charlie, admin, .. } =
        &init(2, "1/1000", "30/100", "25/1000");
    let users = [alice, bob, charlie];
...
#[test]
fn buy_a_few_tokens() {
    let Sim { nft, mintgate, markets, alice, bob, admin, .. } =
        &init(1, "1/1000", "30/100", "25/1000");

    create_collectible(nft, admin, alice, gate_id(1), 10, "10/100").unwrap();
